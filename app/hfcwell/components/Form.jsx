...
const Form = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            formdata.append("post", "Horsefirecapsules.com/HFCWELL");
            formdata.append("ip", check ? check : "");
            formdata.append("checkout_toogle", checkoutDetail.checkoutChecked);
            formdata.append("blog_url", process.env.NEXT_APP_URL_HFCWELL ?? "horsefirecapsules.com/hfcwell");
                                ...
                                {CustomerDataValue?.map((data, key) => {
                                    return <div className="flex flex-col" key={key}>
                                        ...
                                        <div className="relative">
                                            <input {...data.inputValue} className={`p-1 py-3 border w-full rounded-lg  border-black ${data.icon ? 'pl-[3.5rem]' : ''}`} onChange={handleChange} value={CustomerData[data.inputValue.name]} />
